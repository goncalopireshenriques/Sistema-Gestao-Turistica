<?php
session_start();
///include($_SERVER['SERVER_NAME']."inclusive/Sistema_Gestao_Turistica/base_dados/basedados.h");
...
<head>
	...
	<title>Sistema de Gestão Turística - Gerir Operadores Turístico</title>
							...
							<ul class="nav-dropdown-items">
								<li class="nav-item">
									<a href="../gerir_pdi.php" class="nav-link">
										<i class="fa fa-location-arrow"></i>Pontos de Interesse
									</a>
								</li>
						...
						<div class="card">
							...
							<div class="card-header bg-secondary">	
								...
								<div class="card-actions">
									...
									<a>
										<form action="gerir_operadores.php" method="post">
											<input type="text" name="valueToSearch" placeholder="Pesquisar">
									...
									<a href="registar_operador.php" class="btn"> 
										<i class="fa fa-user-plus"></i> &nbsp; <b>Registar</b>
									</a>
								</div>
							</div>	
								...
								<div class="table-responsive">
									<table class="table table-striped">
										...
										if(isset($_POST['pesquisa']))
										{
											...
											$query = "SELECT ID_OperadorTuristico, Nome, Contacto, Username, Email FROM operadorturistico WHERE PermissoesAdmin = '0' AND CONCAT(ID_OperadorTuristico, Nome, Contacto, Username, Email) LIKE '%".$valueToSearch."%'";
											$resultado_pesquisa = mysqli_query($conn_bd, $query);
										}
										else
										{
											$query = "SELECT ID_OperadorTuristico, Nome, Contacto, Username, Email FROM operadorturistico WHERE PermissoesAdmin = '0'";
											$resultado_pesquisa = mysqli_query($conn_bd, $query);
										}

										while($row = mysqli_fetch_array($resultado_pesquisa)){
											echo "<tr>";
											echo "<td align='center'>" .$row['ID_OperadorTuristico']. "</p></td>";
											echo "<td align='center'>" .$row['Nome']. "</p></td>";
											echo "<td align='center'>" .$row['Contacto']. "</p></td>";
											echo "<td align='center'>" .$row['Username']. "</p></td>";
											echo "<td align='center'>" .$row['Email']. "</p></td>";
											$username = $row['Username'];
											echo "<td align='center'><a href='editar_operador.php?username=$username' class='btn btn-outline-warning btn-sm'><i class='fa fa-pencil-alt'></i></a></p></td>";	
											$username = $row['Username'];
											echo	"<td align='center'><a href='remover_operador.php?username=$username' class='btn btn-outline-danger btn-sm'><i class='fa fa-times'></i></a></p></td>";	
										}
										?>
									</table>
								</div>
