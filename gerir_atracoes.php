...
<head>
	...
	<title>Sistema de Gestão Turística - Gerir Atrações Turísticas</title>
						...
						<div class="card">
							<div class="card-header bg-secondary">
								...
								<div class="card-actions">
									...
									<a>
										<form action="gerir_atracoes.php" method="post">
									...
									<a href="adicionar_atracao.php" class="btn">
								...
								<div class="table-responsive">
									<table class="table table-striped">
										...
										if(isset($_POST['pesquisa']))
										{
											...
											$query = "SELECT p.ID_PDI, p.Nome, p.Descricao, p.Latitude, p.Longitude, p.Localizacao, p.QR_Code, p.Audio, p.Categoria, a.ID_Atracao, a.ID_Local, tp.Nome FROM pdi AS p INNER JOIN atracao AS a ON a.ID_PDI = p.ID_PDI INNER JOIN tipopdi AS tp ON tp.ID_TipoPDI = p.ID_Tipo_PDI WHERE CONCAT(p.Nome, p.Descricao, p.Latitude, p.Longitude, p.Localizacao, a.ID_Atracao, tp.Nome) LIKE '%".$valorParaPesquisar."%'";
											$resultado_pesquisa = mysqli_query($conn_bd, $query); //Pesquisa com base no: id atracao, nome, descricao, latitude, longitude, localizacao, tipo
										}
										else
										{
											$query = "SELECT p.ID_PDI, p.Nome, p.Descricao, p.Latitude, p.Longitude, p.Localizacao, p.QR_Code, p.Audio, p.Categoria, a.ID_Atracao, a.ID_Local, tp.Nome FROM pdi AS p INNER JOIN atracao AS a ON a.ID_PDI = p.ID_PDI INNER JOIN tipopdi AS tp ON tp.ID_TipoPDI = p.ID_Tipo_PDI";
											$resultado_pesquisa = mysqli_query($conn_bd, $query);
										}
										
										while($row = mysqli_fetch_array($resultado_pesquisa)){
											echo "<tr>";
											echo "<td align='center'>" .$row['ID_Atracao']. "</td>";
											echo "<td align='center'>" .$row['1']. "</td>"; //nome atracao
											...
											echo "<td align='center'>" .$row['11']. "</td>"; //nome tipo pdi

											$nome_local="SELECT Nome FROM pdi AS p INNER JOIN local AS l ON l.ID_PDI = p.ID_PDI WHERE ID_Local = '$id_local'";
											$query_nome_local = mysqli_query($conn_bd, $nome_local); // nome do local a que esta associada a atracao
											$row_local = mysqli_fetch_assoc($query_nome_local);

											echo "<td align='center'>" .$row_local['Nome']. "</td>";
											...
											echo "<td align='center'><a href='editar_atracao.php?id_pdi=$id_pdi' class='btn btn-outline-warning btn-sm'><i class='fa fa-pencil-alt'></i></a></td>";
											if($perfil==1)
											{
												echo "<td align='center'><a href='./admin/remover_atracao.php?id=$id' class='btn btn-outline-danger btn-sm'><i class='fa fa-times'></i></a></td>";
											}
										}
										?>
									</table>
