...
<head>
	...
	<title>Sistema de Gestão Turística - Processa Adicionar Atração Turística</title>
				...
				<div class="card bg-dark">
					<div class="card-body">
						...
						if (isset($_SESSION['username']) && $_SESSION['perfil'] == '1' || $_SESSION['perfil'] == '0')
						{
							...
							$localAssociado = "SELECT ID_Local FROM local l INNER JOIN pdi p ON p.ID_PDI=l.ID_PDI WHERE Nome='$local'"; 
							$query_local = mysqli_query($conn_bd, $localAssociado); //id do local correpondente ao nome do local
							list($id_local) = mysqli_fetch_row($query_local);

							$insertPDI = "INSERT INTO pdi (Nome, Descricao, Latitude, Longitude, Localizacao, Categoria, ID_Tipo_PDI)
							VALUES ('$nome','$descricao', '$latitude', '$longitude', '$localizacao', 2, '$id_tipo')";

							if(mysqli_query($conn_bd, $insertPDI))
							{	
								$id_pdi = mysqli_insert_id($conn_bd);

								$insertAtracao = "INSERT INTO atracao (ID_Local, ID_PDI) VALUES ('$id_local', '$id_pdi')";
								mysqli_query($conn_bd, $insertAtracao);
								...
								<div class="alert alert-success p-5" align="center">
									<font color="#444" size="6">Atração Turística adicionada com sucesso!</font>
									<?php
							...
							else
							{
								...
								<div class="alert alert-danger p-5" align="center">
									<font color="#444" size="6">Erro ao adicionar Atração Turística!</font>
									<?php
									header("refresh:2;url=adicionar_atracao.php");
									?>
								</div>
								<?php
							}
