...
<head>
	...
	<title>Processa Editar Ponto de Interesse</title>
						...
						if (isset($_SESSION['username']) && $_SESSION['perfil'] == '1' || $_SESSION['perfil'] == '0' )
						{
							...
							{	
								...
								if (!empty($_FILES["imagem"]["name"]) )
								{
									...
									$resultado = mysqli_query($conn_bd, $select_img); //verifica se existe imagem atribuida ao ponto de interesse
									if (mysqli_num_rows($resultado) == 0) // se nao existir, realiza um insert. caso ja existe efetua um update
									{
								...
								if (!empty($_FILES["qr_code"]["name"]))
								{
									...
									$update_qr_code = "UPDATE pdi SET QR_Code = '$upload_directory_qrcode$targetPath' WHERE ID_PDI = '$id_pdi'";
									mysqli_query($conn_bd, $update_qr_code);
								...
								if (!empty($_FILES["audio"]["name"]))
								{
									...
									$update_audio = "UPDATE pdi SET Audio = '$upload_directory_audio$targetPath_audio' WHERE ID_PDI = '$id_pdi'";
									mysqli_query($conn_bd, $update_audio);
								}
								?>
								...
								<div class="alert alert-success p-5" align="center">
									<font color="#444" size="6">Ponto de Interesse editado com sucesso!</font>
									<?php
							...
							else
							{
								...
								<div class="alert alert-danger p-5" align="center">
									<font color="#444" size="6">Erro ao editar Ponto de Interesse!</font>
									<?php
									header("refresh:2;url=editar_pdi.php?id_pdi=$id_pdi");
									?>
								</div>
								<?php
							}
