...
<head>
	...
	<title>Sistema de Gestão Turística - Associar Tipo de Acessibilidade</title>
	...
	<div class="page-wrapper">
		...
		<div class="main-container">
			...
			<div class="content">
				<div class="row">
					<div class="col-md-6" >
						<form class="card"  method="POST" action="processa_associar_acessibilidade.php">
							<div class="card-header bg-secondary">
								<b>Formulário de Associação de Acessibilidade(s)</b>
							</div>
							...
							<div class="card-body" >
								<div class="row">
									<div class="col-md-12">
										<label for="multi-select" class="require"> Acessibilidade(s): Pressione Ctrl para multipla seleção</label>
										<div class="input-group mb-3">
											<div class="input-group-prepend">
												<span class="input-group-text"><i class="fa fa-tags"></i></span>
											</div>
											<?php
											$acessibilidade="SELECT ID_Acessibilidade FROM acessibilidade WHERE ID_Acessibilidade NOT IN (SELECT ID_Acessibilidade FROM pdi_acessibilidade WHERE ID_PDI='$id_pdi') ORDER BY ID_Acessibilidade";
											$query = mysqli_query($conn_bd, $acessibilidade);
											$option = '';
											while($row = mysqli_fetch_assoc($query))
											{	
												$id_acessibilidade = $row['ID_Acessibilidade'];
												$select_nome="SELECT Nome FROM acessibilidade WHERE ID_Acessibilidade='$id_acessibilidade'";
												$nome = mysqli_query($conn_bd, $select_nome);
												$row2 = mysqli_fetch_assoc($nome);
												$option .= '<option value = "'.$row2['Nome'].'">'.$row2['Nome'].'</option>';		
											}
											?>
											<select name="acessibilidade[]" required id="acessibilidade" class="form-control" multiple="multiple">
												<?php echo $option; ?>
											</select>
										</div>
									</div>
